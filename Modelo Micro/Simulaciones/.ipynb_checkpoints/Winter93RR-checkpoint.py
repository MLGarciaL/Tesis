...
def A_it1(A_it, d_imt, d_int, t, Amax, mu, sigma):
    patent = 0
    innov_it = np.exp(innov(mu, sigma, t, A_it))    
...
def NWCap12(t = 100, n = 32, prop = 0.5, patentime = 5, I = 0):
    ...
    for t in range(0, t + 1):
        ...
        for i in range(0,n):
            if Viva[i] >= 0:
                ...
                A_it[t+1][i], patent_it[t][i] = A_it1(A_it[t][i], dimt_it[t][i], dint_it[t][i], t, Amax, mu, sigma)
        ...
        for i in range(innovators):
            A0 = np.exp(innov(mu,sigma,t,.1333))  #.1333 Public Knowledge
