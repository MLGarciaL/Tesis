...
def profit_rate(A_it, p_t, r_im, r_in, c, I, CT):
    return p_t*A_it - c + CT*I - r_im - r_in
...
def A_it1(A_it, d_imt, d_int, t, Amax, mu, sigma, RR):
    patent = 0
    if RR == False:
...
def adaptativeRD(X_it, pi_t, r_mt, r_nt, r_imt, r_int, beta):
    if X_it < pi_t and np.random.choice([0, 1]) == 1:
        sigma_m = 0.0004
        sigma_n = 0.002
        u_int = max(np.random.normal(0, sigma_n),0)
        u_imt = max(np.random.normal(0, sigma_n),0)
...
def NWCap12(t = 3, n = 32, prop = 0.5, patentime = 5, I = 0, Instmu = 0.005, Instsigma = 0.005, CT = 0.01, RR = False):
    ...
    K0 = 139.58
    A0 = 0.16
    mu = np.log(0.135*(1 + Instmu*I))
    sigma = 0.1177*(1 - Instsigma*I)
    X0 = 0.001
    Kmin = 10
    Xmin = -0.051
    XDelta = 0.001
    ...
    for t in range(0, t + 1):
        ...
        for i in range(0,n):
            if Viva[i] >= 0:
                ...
                pi_it[t][i] = profit_rate(A_it[t][i], p_t[t], r_imt[t][i], r_int[t][i], c, I, CT)
